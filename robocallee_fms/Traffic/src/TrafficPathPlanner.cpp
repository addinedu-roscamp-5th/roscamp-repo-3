#include "include/TrafficAdapter.hpp"
#include <queue>
...
struct TrafficNode {
    ...
    bool operator>(const TrafficNode& other) const {
        return cost > other.cost;
    }
};

// ========== TrafficSolver 생성자 구현 ==========
TrafficSolver::TrafficSolver(
...
// ========== 충돌 탐지 ==========
std::vector<Conflict> TrafficSolver::detectCollisions(const std::vector<std::vector<Position>>& paths) const {
    std::vector<Conflict> conflicts;
    size_t max_t = 0;
...
// ========== 표준 Conflict 분할 ==========
std::vector<Constraint> TrafficSolver::standardSplitting(const Conflict& conflict) const {
    std::vector<Constraint> constraints;
...
// ========== Disjoint Splitting ==========
std::vector<Constraint> TrafficSolver::disjointSplitting(const Conflict& conflict) const {
    // 간단하게 표준 분할과 동일하게 리턴 (실제 구현시 random이나 heuristic 선택 가능)
    return standardSplitting(conflict);
}

// ========== Traffic 메인 알고리즘 ==========
std::vector<std::vector<Position>> TrafficSolver::findSolution(bool disjoint) {
    AStarPlanner planner(map_);
    std::priority_queue<TrafficNode, std::vector<TrafficNode>, std::greater<>> open;
    TrafficNode root;
    root.cost = 0;
    root.id = 0;
    ...
    while (!open.empty()) {
        TrafficNode curr = open.top(); open.pop();

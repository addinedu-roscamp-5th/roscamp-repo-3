...
class MapWidget(QLabel):
    ...
    def update_map(self):
        ...
        for rid in range(1, NUM_ROBOTS + 1):
            if rid not in self.robot_paths or len(self.robot_paths[rid]) < 2:
                continue

            pts = self.robot_paths[rid]

            # 선 색상/두께/투명도 (alpha=0.3)
            base = robot_colors[rid % len(robot_colors)]
            ...
            poly = QPolygon()
            for (wx, wy) in pts:
                px = self._wx_to_px(wx, rect)
                py = self._wy_to_py(wy, rect)
                poly.append(QPoint(px, py))
            painter.drawPolyline(poly)

            # 시작/끝 마커 (시작=원, 끝=사각형)
            mark_color = QColor(base)
            mark_color.setAlphaF(0.7)
            painter.setPen(Qt.NoPen)
            painter.setBrush(QBrush(mark_color))

            sx = self._wx_to_px(pts[0][0], rect)
            sy = self._wy_to_py(pts[0][1], rect)
            painter.drawEllipse(sx - 6, sy - 6, 12, 12)   # 시작점

            ex = self._wx_to_px(pts[-1][0], rect)
            ey = self._wy_to_py(pts[-1][1], rect)
            painter.drawRect(ex - 6, ey - 6, 12, 12)  
    ...
    def update_robot_path(self, rid, path):
        # for k in self.robot_paths:
        #     self.robot_paths[k] = []
...
class IntegratedRosThread(QThread):
    ...
    robot_path_update = pyqtSignal(int,object)
    ...
    def setup_aruco(self):
        ...
        try:
            ...
            candidates.append(FilePath('/home/addinedu/ROS2_Jazzy_Study/src/aruco_marker_pkg/include'))
            candidates.append(FilePath(__file__).resolve().parent / 'include')
    ...
    def setup_robot_subscribers(self, robot_id):
        ...
        if robot_id not in self.subscribers:
            ...
            self.subscribers[robot_id]['path'] = self.node.create_subscription(
                PathMsg, f'/path{robot_id}',
                lambda msg, rid=robot_id: self.robot_path_update.emit(rid, msg),10)
    ...
    def make_path_cb(self, robot_id):
        ...
        def callback(msg):
            ...
            self.node.get_logger().info(f':철도:  로봇 {robot_id} 경로 수신: {len(path_points)} 점')
        return callback
...
class IntegratedMainWindow(QMainWindow):
    ...
    def connect_ros_signals(self):
        ...
        self.ros_thread.robot_path_update.connect(self.update_robot_paths)

    def update_robot_paths(self, rid, msg):
        path_points = []
        for pose in msg.poses:
            x = pose.pose.position.x
            y = pose.pose.position.y
            path_points.append((x, y))
        self.map_widget.update_robot_path(rid, path_points)
        self.get_logger().info(f':Path:  로봇 {rid} 경로 수신: {len(path_points)} 점')

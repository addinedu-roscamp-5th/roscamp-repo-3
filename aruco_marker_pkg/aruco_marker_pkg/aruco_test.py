...
class ArucoOdometryPublisher(Node):
    ...
    def process_frame(self):
        ...
        for marker in detected:
            ...
            cv2.putText(frame, f"Raw XYZ: ({raw_x:.3f}, {raw_y:.3f}, {raw_z:.3f})", (cx -10, cy + 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 128, 0), 2)
            cv2.putText(frame, f"Yaw: {yaw:.3f}", (cx -10, cy + 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 128, 0), 2)
            ...
            if marker_id in self.robot_states and self.robot_states[marker_id].initialized:
                ...
                cv2.putText(frame, f"Speed: {speed:.2f} m/s", (cx -10, cy + 60),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                cv2.putText(frame, f"Ang vel: {state.vyaw:.2f} rad/s", (cx -10, cy + 120),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

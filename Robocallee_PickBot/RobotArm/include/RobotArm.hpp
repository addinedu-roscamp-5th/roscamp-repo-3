...
namespace RA
{
    ...
    public:
        ...
        bool SetCoords(const std::vector<float>& Coords,const uint8_t speed, const uint8_t Mode = 1);
        bool GetCoords(std::vector<float>& Coords);
        bool SetEndCooriSys(uint8_t tool);

        bool SetGrippeerState(Gripper girpper, const uint8_t speed);
        bool GetGripperValue(int& val);
    };
}

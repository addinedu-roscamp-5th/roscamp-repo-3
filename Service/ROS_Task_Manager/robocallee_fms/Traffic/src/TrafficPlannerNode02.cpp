...
void TrafficPlannerNode::initializeParameters() {
    ...
    declare_parameter("timeout_seconds", 10.0);  // 10초로 변경
    declare_parameter("resolution", 0.2);        // 20cm로 변경
...
void TrafficPlannerNode::initializeMap() {
    ...
    RCLCPP_INFO(get_logger(), "🗺️  교통 규칙 맵 초기화 완료 (20cm 해상도, 12x7 맵)");
}
...
void TrafficPlannerNode::planAndAssignPaths() {
    ...
    // 20cm 해상도에 맞게 조정된 제약조건
    std::vector<Constraint> custom_constraints = {
        {0, 2, { {2, 4} }},  // 로봇 0이 시간 2에 위치 (2,4)에 있으면 안됨
    };
    RCLCPP_INFO(get_logger(), "🚀 Traffic-aware CBS 솔버 시작 - 로봇 %d개 (20cm 해상도)", num_robots_);
    
    auto paths = planner_->planPaths(starts, goals, true, custom_constraints);
    ...
    for (int id = 1; id <= num_robots_; ++id) {
        ...
        RCLCPP_INFO(get_logger(), "[%d] Traffic-aware waypoints 생성 완료: %zu개 (20cm 해상도)", id, rd.waypoints.size());
    }
}
...
std::vector<Position> TrafficPlannerNode::generateStartPositions() {
    ...
    if (custom_starts.size() == static_cast<size_t>(num_robots_ * 2)) {
        ...
        RCLCPP_INFO(get_logger(), "📍 커스텀 시작점 사용 (20cm 해상도)");
        return starts;
    }

    // 12x7 맵에 맞게 조정된 기본 시작점 생성
    std::vector<Position> starts;
    int start_col = 10;  // 우측 가장자리 (테두리 이전)
    
    for (int i = 0; i < num_robots_; ++i) {
        // 장애물이 있는 행(2,3,4)을 피해서 안전한 행만 사용
        int current_row;
        if (i == 0) current_row = 1;  // 첫 번째 로봇
        else if (i == 1) current_row = 5;  // 두 번째 로봇
        else current_row = 1 + (i % 2) * 4;  // 세 번째 이후는 1,5 반복
        
        starts.push_back({current_row, start_col});
    }
    
    RCLCPP_INFO(get_logger(), "📍 교통 흐름 기반 기본 시작점 생성 완료 (%d개, 20cm 해상도)", num_robots_);
    return starts;
}
...
std::vector<Position> TrafficPlannerNode::generateGoalPositions() {
    ...
    if (custom_goals.size() == static_cast<size_t>(num_robots_ * 2)) {
        ...
        RCLCPP_INFO(get_logger(), "🎯 커스텀 목표점 사용 (20cm 해상도)");
        return goals;
    }

    // 12x7 맵에 맞게 조정된 기본 목표점 생성
    std::vector<Position> goals;
    int goal_col = 1;   // 좌측 가장자리 (테두리 다음)
    ...
    for (int i = 0; i < num_robots_; ++i) {
        ...
        goals.push_back({current_row, goal_col + i});  // 목표점은 약간씩 다른 열에 배치
    }
    
    RCLCPP_INFO(get_logger(), "🎯 교통 패턴 기반 기본 목표점 생성 완료 (%d개, 20cm 해상도)", num_robots_);
    return goals;
}
...
std::vector<WaypointWithYaw> TrafficPlannerNode::convertPathToWaypoints(const std::vector<Position>& path) {
    ...
    for (size_t i = 0; i < path.size(); ++i) {
        ...
        double yaw = M_PI / 2.0;  // 기본값 (북쪽 방향)
        
        if (i > 0) {
            // 이전 웨이포인트와의 차이로 방향 계산
            double dx = x - waypoints[i-1].x;
            double dy = y - waypoints[i-1].y;
            
            // 방향이 있는 경우에만 yaw 업데이트
            if (std::abs(dx) > 1e-6 || std::abs(dy) > 1e-6) {
                yaw = std::atan2(dy, dx);
            } else {
                // 같은 위치면 이전 yaw 유지
                yaw = waypoints[i-1].yaw;
            }
        }
...
void TrafficPlannerNode::publishPath(int id) {
    ...
    RCLCPP_INFO(get_logger(), "[%d] 🛤️  교통 규칙 기반 경로 publish 완료 (%zu waypoints, 20cm 해상도)", id, rd.waypoints.size());
}
...
void TrafficPlannerNode::handlePose(int id, geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    ...
    if (dx * dx + dy * dy < RobotData::THRESH_SQ) {
        RCLCPP_INFO(get_logger(), "✅ [%d] 교통 규칙 준수하며 wp[%zu] 도달! (20cm 해상도)", id, rd.waypoint_index);
...
void TrafficPlannerNode::sendGoalPose(int id) {
    ...
    RCLCPP_INFO(get_logger(),
        "[%d] 🟢 교통 규칙 기반 goalpose publish → wp[%zu] (%.2f, %.2f) [20cm 해상도]", 
...
void TrafficPlannerNode::advanceWaypoint(int id) {
    ...
    if (rd.waypoint_index < rd.waypoints.size()) {
        ...
        RCLCPP_INFO(get_logger(), "🎉 [%d] 교통 규칙을 준수하며 모든 waypoint 완료 (20cm 해상도)", id);
    }
}

...
// AStarPlanner 구현
AStarPlanner::AStarPlanner(const std::vector<std::vector<bool>>& map) 
...
bool AStarPlanner::isValidPosition(const Position& pos, const std::vector<std::vector<int>>& grid) const {
    return pos.x >= 0 && pos.y >= 0 && 
           static_cast<size_t>(pos.y) < grid.size() && 
           static_cast<size_t>(pos.x) < grid[0].size();
}

bool AStarPlanner::isValidPosition(const Position& pos, const std::vector<std::vector<bool>>& grid) const {
    return pos.x >= 0 && pos.y >= 0 && 
           static_cast<size_t>(pos.y) < grid.size() && 
           static_cast<size_t>(pos.x) < grid[0].size();
}

std::vector<Position> AStarPlanner::reconstructPath(std::shared_ptr<Node> node) const {
...
bool AStarPlanner::isConstrained(const Position& curr, const Position& next, int timestep, 
...
TrafficPlanner::TrafficPlanner(const std::vector<std::vector<bool>>& map, Log::Logger::s_ptr log)
    : map_(map), log_(log) {
    std::string msg = "TrafficPlanner 초기화 완료. 맵 크기: " + 
...
bool TrafficPlanner::validateInput(const std::vector<Position>& starts, const std::vector<Position>& goals) {
...
bool TrafficPlanner::isValidPosition(const Position& pos) const {
    return pos.x >= 0 && pos.y >= 0 && 
           static_cast<size_t>(pos.y) < map_.size() && 
           static_cast<size_t>(pos.x) < map_[0].size();
}

std::vector<std::vector<Position>> TrafficPlanner::runCBS(
    const std::vector<Position>& starts,
    const std::vector<Position>& goals,
    bool use_disjoint_splitting
) {
...
CBSNode TrafficPlanner::createRootNode(const std::vector<Position>& starts, 
                      const std::vector<Position>& goals,
...
bool TrafficPlanner::expandNode(const CBSNode& curr, 
               std::priority_queue<CBSNode, std::vector<CBSNode>, std::greater<>>& open,
               const std::vector<Position>& starts,
               const std::vector<Position>& goals,
               AStarPlanner& planner,
...
int TrafficPlanner::calculateTotalCost(const std::vector<std::vector<Position>>& paths) const {
    int total = 0;

...
struct WaypointWithYaw {
    ...
    WaypointWithYaw(double x_val, double y_val, double yaw_val) 
        : x(x_val), y(y_val), yaw(yaw_val) {}
};
...
struct RobotData {
    ...
    // 20cm 해상도에 맞게 threshold 조정
    // 해상도가 20cm이므로 8cm threshold 사용 (해상도의 40%)
    static constexpr double THRESHOLD = 0.08;  // 8cm
